...
public class CustomLogger {
    ...
    public static void logExternalRedis(String operation, String key, String status, String message) {
        try {
            Map<String, Object> logMap = new HashMap<>();
            logMap.put("timestamp", LocalDateTime.now().toString());
            logMap.put("level", "INFO");
            logMap.put("logType", "EXTERNAL_REDIS_" + operation.toUpperCase());
            logMap.put("traceId", MDC.get("traceId"));
            logMap.put("system", "Redis");
            logMap.put("operation", operation);
            logMap.put("target", key);
            logMap.put("status", status);
            if (message != null)
            ...
            errorLogger.error("Failed to log Redis external access", e);
        }
    }
}